            ...
            <div class="mt-3">
                <div class="grid grid-cols-3 ">
                    <div class="bg-gray-200 p-3">
                        ...
                        <ul class="mt-5  grid grid-cols-1">
                            @foreach($modules as $module)
                                <button type="button" wire:click.prevent="selectModule({{$module->id}})" wire:key="{{$module->id}}" >
                                    <li
                    ...
                    <div class="bg-gray-300 p-3">
                        ...
                        @if($subModules)
                            <span class="text-blue-900 font-light text-sm capitalize">For: {{$selectedModule->name}}</span>
                            <ul class="mt-5  grid grid-cols-1">
                                @foreach($subModules as $subModule)
                                    <button type="button"  wire:key="{{$subModule->id}}" wire:click="selectSubModule({{$subModule->id}})">
                                        <li
                                            class="text-left mt-2 capitalize text-blue-600 underline"
                    ...
                    <div class="bg-gray-400 p-3">
                        ...
                        @if($applications)
                            <span class="text-blue-900 font-light text-sm capitalize">For: {{$selectedSubModule->name}}</span>
                            <ul class="mt-5  grid grid-cols-1">
                                @foreach($applications as $application)
                                    <button type="button"  wire:key="{{$application->id}}" wire:click="selectSubModule({{$application->id}})">
                                        <li
                                            class="text-left mt-2 capitalize text-blue-600 underline"
